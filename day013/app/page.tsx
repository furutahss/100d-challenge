...
export default function Home() {
  ...
  const answer = (food: Animal) => { if (picked) return; setPicked(food.food === current.food ? current.name : food.name); if (food.food === current.food) setScore((s) => s + 1); };
  const next = () => { if (index === 4) { setStarted(false); setFinished(true); } else { setIndex((i) => i + 1); setPicked(null); } };

  if (finished) return <main className="home"><div className="hero-image" style={{ backgroundImage: `url('${assetBasePath}/hero.png')` }} /><section className="intro"><p className="eyebrow">ANIMAL CAFÉ <span>RESULT</span></p><h1>おつかれさま！<br /><em>{score}もん</em> せいかい</h1><p className="lead">どうぶつたちは にこにこ。<br />また いつでも あそびにきてね。</p><button className="start" onClick={start}>もういちど あそぶ <span>↻</span></button></section></main>;
  if (!started) return <main className="home"><div className="hero-image" style={{ backgroundImage: `url('${assetBasePath}/hero.png')` }} /><section className="intro"><p className="eyebrow">ANIMAL CAFÉ <span>DAY 013</span></p><h1>どうぶつに<br /><em>ごはんを</em>あげよう</h1><p className="lead">おなかをすかせた なかまたちに<br />すきな たべものを とどけてあげてね。</p><button className="start" onClick={start}>あそびにいく <span>→</span></button><p className="note">全5問 ／ ゆっくり考えて大丈夫</p></section></main>;
